...
import { AnswerRadioGroupComponent } from '../answer-radio-group/answer-radio-group.component';
...
export class ContainerComponent implements OnInit, AfterViewInit{
  ...
  @ViewChild(AnswerRadioGroupComponent) child;
  ExistPreviousQuestion: boolean;
  ...
  constructor(private surveyService: SurveyService) {
    ...
    this.ExistPreviousQuestion = false
   }
  ...
  ngAfterViewInit(){
    //this.previousAnswerId = this.child.answerId;
    //console.log("ngAfterViewInit"+this.child.answerId);
  }
      ...
      if (this.IsNext) {
        ...
        if (data) {
          ...
          if (statusCode === 200) {
            ...
            if (this.currentQuesionFull.question.questionId == '1.1'){
              this.ExistPreviousQuestion = false;
              console.log("primer pregunta "+this.currentQuesionFull.question.questionId+
                "ExistPreviousQuestion "+this.ExistPreviousQuestion) 
            }else{
              this.ExistPreviousQuestion = true;
              console.log("no es 1er pregunta "+this.currentQuesionFull.question.questionId+
                "ExistPreviousQuestion "+this.ExistPreviousQuestion) 
            }
  ...
  async outputResponse(response: QuestionFullResultModel) {
    /*if (response.IsBackQuestion == true) {
      this.comeToTheBackQuestion();
      //console.log("container desde boton regresar");
      //console.log("IsReturnQuestion "+this.IsReturnQuestion)
      //console.log("response.question.id: " + response.question.questionId + " to back "+this.toBack);
      
    }else{*/
      console.log("container desde boton guardar");
   ...
   async saveAnswer(answer: SaveAnsweredQuestionParamsModel) {
    ...
    let data: ReturnServiceModel;
    console.log("saveAnswer " + answer.SaveAnsweredQuestion[0].AnswerText);
   ...
   /*
   async backQuestion(questionId:number){
    
    let questionSearched = questionId;
    
    this.Id = 0;
    console.log("backQuestion buscar pregunta "+questionSearched+"\n this.Id"+this.Id);
    this.loading = true;
    this.error = false;
    ...
    do {
      ...
      if (data1) {
        ...
        if (statusCode === 200) {
          ...
          if (questionBackNextModel.exist) {
            ...
            this.Id = questionBackNextModel.id;
            this.SectionId = questionBackNextModel.sectionId;
            console.log("backQuestion this.Id "+this.Id); 
          }
          
        }
      }
      console.log("this.currentQuesion.id "+this.currentQuesion.id);
      if (this.IsNext && this.currentQuesion.id != questionSearched) {
        let data: ReturnServiceModel;
        data = await this.surveyService.getQuestionById(this.Id, this.SectionId, this.userId).toPromise();
        if (data) {
          const statusCode = data.statusCode;
      ...
      if (this.currentQuesion.id == questionSearched){
        toNext = false;
        console.log("Se econtró la pregunta buscada "+this.currentQuesion.id);
      }
    } while (toNext);
   ...
   */
   ...
   async comeToTheBackQuestion(){
    ...
    console.log("comeToTheBackQuestion GetIdPreviousQuestion"+
    "\n prevQuestion id: " + prevQuestion.id + " prevQuestion.sectionId "+prevQuestion.sectionId+
    "\n prevQuestion.questionId "+prevQuestion.questionId);
    this.backQuestion(prevQuestion.id);
    ...
    data = await this.surveyService.getQuestionById(prevQuestion.id, prevQuestion.sectionId, this.userId).toPromise();
        if (data) {
          ...
          if (statusCode === 200) {
            ...
            if (this.currentQuesion.questionTypeId == 2 || this.currentQuesion.questionTypeId == 3){
              this.previousAnswerId = this.currentAnswered[0].answerId;
              console.log("Come to the back question "+this.currentQuesionFull.answered[0].answerText)
